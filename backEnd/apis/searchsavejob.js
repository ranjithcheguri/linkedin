...
router.put('/searchsavejob', function (req, res) {
    console.log("Inside saveJob : ");
    //$addToSet --> to push unique values to array
    //$push --> to push all values.
    applicantProfiles.find({ email: req.body.email }, { $forEach: { savedJobs: req.body.saveJob } }, { upsert: true }, function (err, result) {
        if (err) {
